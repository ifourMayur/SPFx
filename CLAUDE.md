...
- There is no `npm test` script; testing runs through `heft test`, which builds first, then runs Jest via `@rushstack/heft-jest-plugin`. Results land in `jest-output/` (JUnit XML + coverage).
