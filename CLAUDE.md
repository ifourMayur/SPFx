...
[README.md](README.md) is the **untouched Yeoman generator template** ("Short summary on functionality and used technologies", placeholder author table, invented version history). It describes nothing about this solution — don't mine it for facts or treat its steps as the current workflow; this file and the source comments are the documentation. The same scaffolding survives in two places in the code: the "Well done, …" block at the end of `XsProject.render` (see "Rendering and routing") and `XsProjectWebPart._getEnvironmentMessage`, the Teams/Outlook/office.com banner — nothing ported depends on either.
...
**Only the web part uses localized strings.** `XsProjectWebPart.ts` is the single consumer of `import * as strings from 'XsProjectWebPartStrings'` (property pane labels, the environment banner), and adding or renaming one is two coordinated edits: the value in `src/webparts/xsProject/loc/en-us.js` and the member in `loc/mystrings.d.ts` — the `.d.ts` is what declares the virtual module, so a string added to only one of them either fails to compile or renders `undefined`. The module name is bound to the file in `config/config.json` (`localizedResources`), and `config/typescript.json` copies `webparts/*/loc/*.js` through as a static asset. Ported UI text does **not** go here: it is written inline, verbatim from the reference's resource files (see "The project form").

### Layout and styling

The web part is a **full-page application, not a card on a page**, and nothing inside the bundle can make that true on its own — `domElement` is nested in SharePoint's canvas (`.CanvasZone` / `.CanvasSection` / `.ControlZone`), which caps the chain *around* it at a fixed pixel width and centres it with auto margins, no matter what the app's own stylesheets say. Three pieces cooperate:

- `XsProjectWebPart.manifest.json` sets `supportsFullBleed: true`, so an author can place the web part in a full-width section, where SharePoint puts no width cap on the canvas at all.
- `XsProjectWebPart._releaseWidthCaps` covers the normal content section, which still has to work. It is deliberately narrow in what it touches: only ancestors of *this* web part, only their `max-width`, and only when that resolves to a fixed `px` value (a percentage cap is a proportion of a parent that is itself being widened, not a centring device). Every original inline value is remembered and restored in `onDispose`, so removing the web part leaves the page exactly as it was.
- `XsProjectWebPart._measureAvailableHeight` publishes the height left below the web part as a `--xsProjectAvailableHeight` custom property, which `XsProject.module.scss` gives `.appLayout` as `min-height` (fallback `70vh`, which is what jsdom tests see). It has to be **measured, not written as `100vh`**: the page chrome above the web part is a different height on a team site, a communication site, a Teams tab and the workbench, and changes again in page edit mode. Re-measured on `window.resize`, registered once however often SPFx re-renders.

What that implies for new markup and styles:

- Page roots carry `box-sizing: border-box; width: 100%` and **no width, max-width or auto margin** — the app fills whatever it is handed.
- **Nothing clips at the root.** `overflow: hidden` was removed from `.xsProject` on purpose: it also clipped the `MultiSelect` panel and the row menus that open over the page. Anything that can overflow constrains itself instead — the listing table sits in its own `overflow-x: auto` wrapper.
- One breakpoint, `max-width: 767px`, is shared by `XsProject`, `Menu` and `ProjectList`: the two-column shell stacks, the sidebar becomes a full-width band rather than a 48px rail with nothing beside it, and the `space-between` toolbars wrap.
- Colours go through SPFx theme tokens (`"[theme:neutralLight, default: #e3e6ea]"`) rather than literals. `*.scss.ts` typings are generated by the sass task and gitignored — never hand-edit or commit them (`.vscode/settings.json` also hides them from the file explorer, so they look absent in the IDE while the imports they type resolve fine).
...
Three coordinated edits and nothing else: a path in `ROUTE_PATHS` (`src/models/Navigation.ts`), a `<Route>` in `AppShell.tsx`, and an entry in `MENU_ITEMS` (`Menu.tsx`) — a leaf for a top-level item, or a `children` entry for a submenu. `Menu` is data-driven and rendered once by the shell, so no page carries navigation markup. `Document`, `Search` and `Suppliers` are "Coming soon" placeholders in exactly this shape, waiting on a backing endpoint. A new page's root class follows "Layout and styling" above: `box-sizing: border-box; width: 100%`, nothing that caps or centres it.
...
The manifest's `supportedHosts` is not just `SharePointWebPart`: it also lists `SharePointFullPage`, `TeamsTab` and `TeamsPersonalApp` (Teams app icons live in `teams/`), so the same bundle can be surfaced as a full-page app or inside Teams — another reason the available height is measured rather than assumed (see "Layout and styling"). Nothing in the repo deploys the result: there is no CI config and no deploy script, only `heft package-solution`, which writes the `.sppkg`.
